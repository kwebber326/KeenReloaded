...
namespace KeenReloaded.Framework.Enemies
{
    public class GnosticeneAncient : DestructibleObject, IEnemy, IUpdatable, IGravityObject, ISprite, IStunnable, ICreateRemove, IZombieBountyEnemy
    {
        ...
        public GnosticeneAncient(SpaceHashGrid grid, Rectangle hitbox, CommanderKeen keen)
            : base(grid, hitbox)
        {
            if (keen == null)
        ...
        private void Initialize()
        {
            ...
            if (GetTopMostLandingTile(_currentVerticalVelocity) == null)
            {
                this.Fall();
            }
        }
        ...
        public void Fall()
        {
            ...
            {
                this.HitBox = new Rectangle(_sprite.Location, _originalHitboxSize);
            }
        ...
        private void AddDestroyersForItems(List<Item> stealableItems)
        {
            ...
            {
                GnosticeneItemDestroyer destroyer = new GnosticeneItemDestroyer(_collisionGrid, item.HitBox, item);
                destroyer.Remove += new EventHandler<KeenEventArgs.ObjectEventArgs>(destroyer_Remove);
        ...
        void destroyer_Remove(object sender, KeenEventArgs.ObjectEventArgs e)
        {
            OnRemove(e);
        }
        ...
        {
            int deceleration = GRAVITY_ACCELERATION;
        ...
        protected virtual CollisionObject GetTopMostLandingTile(List<CollisionObject> collisions)
        {
            ...
            var items = collisions.Where(c => c is DebugTile || c is PlatformTile || c is PoleTile).ToList();
        ...
        private void UpdateSprite()
        {
            ...
            {
                case GnosticeneAncientState.LOOKING:
                    this.Sprite.Image = _direction == Enums.Direction.LEFT ? Properties.Resources.keen4_gnosticene_ancient_look_left
                ...
                case GnosticeneAncientState.FALLING:
                    ...
                    if (_currentJumpSpriteChangeDelayTick++ == JUMP_SPRITE_CHANGE_DELAY)
                    {
                        ...
                        this.Sprite.Image = spriteSet[_currentJumpImage++];
                    }
                    break;
                case GnosticeneAncientState.VANISHING:
                    if (_currentVanishSpriteChangeDelayTick++ == VANISH_SPRITE_CHANGE_DELAY)
                    {
                        ...
                        this.Sprite.Image = _vanishingImages[_currentVanishImage];
                    }
                    break;
                case GnosticeneAncientState.APPEARING:
                    if (_currentVanishSpriteChangeDelayTick++ == VANISH_SPRITE_CHANGE_DELAY)
                ...
                case GnosticeneAncientState.STUNNED:
                    if (_currentStunSpriteChangeDelayTick++ == STUN_SPRITE_CHANGE_DELAY)
                    {
                        ...
                        this.Sprite.Image = _stunImages[_currentStunImage++];
                    }
                    break;
            }
        }
        ...
        public void Stun()
        {
            if (this.MoveState != GnosticeneAncientState.STUNNED)
            {
                ...
                this.Sprite.Image = Properties.Resources.keen4_gnosticene_ancient_stun1;
                this.HitBox = new Rectangle(_sprite.Location, new Size(_sprite.Width, _sprite.Height + 30));
                this.UpdateCollisionNodes(Enums.Direction.DOWN);
                _currentVerticalVelocity = INITIAL_FALL_VELOCITY;
            }
