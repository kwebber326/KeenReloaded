...
namespace KeenReloaded.Framework.Enemies
{
    public class SkyPest : CollisionObject, IUpdatable, IEnemy, IMoveable, ISprite, ISquashable, IZombieBountyEnemy
    {
        ...
        public SkyPest(SpaceHashGrid grid, Rectangle hitbox, CommanderKeen keen)
            : base(grid, hitbox)
        {
            if (keen == null)
            {
        ...
        private void Initialize()
        {
            ...
            SetRandomInitialHorizontalFlyingDirection();
        }
        ...
        private void SetRandomInitialHorizontalFlyingDirection()
        {
                ...
                case SkyPestState.FLYING:
                    ...
                    if (this.Direction == Enums.Direction.DOWN_LEFT || this.Direction == Enums.Direction.UP_LEFT)
                    {
                        if (_currentFlyingImage >= _leftFlyingImages.Length)
                            _currentFlyingImage = 0;
                        this.Sprite.Image = _leftFlyingImages[_currentFlyingImage];
                    }
                    else
                    {
                        if (_currentFlyingImage >= _rightFlyingImages.Length)
                            _currentFlyingImage = 0;
                        this.Sprite.Image = _rightFlyingImages[_currentFlyingImage];
                    }
                    break;
                case SkyPestState.STANDING:
                    if (_currentStandingImage == 0 || _currentStandingImage == 8)
                    {
                        this.Sprite.Image = _standingImages[0];
                    }
                    else
                    {
                        this.Sprite.Image = _standingImages[_currentStandingImage];
                    }
                    break;
                case SkyPestState.SQUASHED:
                    this.Sprite.Image = Properties.Resources.keen4_skypest_dead;
                    break;
            }
        }
        ...
        public event EventHandler<KeenEventArgs.ObjectEventArgs> Squashed;
        public event EventHandler<ObjectEventArgs> Killed;

        protected void OnSquashed()
        {
            Squashed?.Invoke(this, new KeenEventArgs.ObjectEventArgs() { ObjectSprite = this });
            OnKilled();
        }
