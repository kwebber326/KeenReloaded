...
namespace KeenReloaded.Framework.Enemies
{
    public class Orbatrix : CollisionObject, IUpdatable, ISprite, IEnemy
    {
        ...
        public Orbatrix(SpaceHashGrid grid, Rectangle hitbox, CommanderKeen keen)
            : base(grid, hitbox)
        {
            if (keen == null)
        ...
        private void Initialize()
        {
            ...
            _lookSprites = SpriteSheet.OrbatrixLookImages;
            _lookLeftSprites = SpriteSheet.OrbatrixLookLeftImages;
            _lookRightSprites = SpriteSheet.OrbatrixLookRightImages;
            _attackSprites = SpriteSheet.OrbatrixAttackImages;

            int directionVal = _random.Next(0, 2);
            _direction = directionVal == 0 ? Direction.LEFT : Enums.Direction.RIGHT;
            UpdateSprite();
            this.Look();
        }
        ...
        private void Move()
        {
            ...
            ////check for edges
            //if (IsOnEdge(this.Direction) && !_isMovingFromAttack)
            //{
            //    this.BasicFall(1);
            //    if (IsOnEdge(this.Direction))
            //    {
        ...
        private void UpdateSprite()
        {
            switch (_state)
            {
                ...
                case OrbatrixState.PREPARING_ATTACK:
                case OrbatrixState.PREPARING_MOVE:
                    _sprite.Image = Properties.Resources.keen6_orbatrix_close;
                    break;
                case OrbatrixState.ATTACKING:
                    ...
                    break;
                case OrbatrixState.ASCENDING:
                    _sprite.Image = Properties.Resources.keen6_orbatrix_attack1;
                    break;
            }
        }
