        ...
        public SpaceHashGrid(int width, int height, int hashWidth, int hashHeight)
        {
            ...
            for (int x = 0; x < width; x += hashWidth)
            {
                for (int y = 0; y < height; y += hashHeight)
                {
                    ...
                    node.Display = new PictureBox();
                    node.Display.BorderStyle = BorderStyle.Fixed3D;
                    node.Display.Location = node.HashBox.Location;
                    node.Display.Size = node.HashBox.Size;
                    node.Display.BackColor = Color.Gray;
                    _nodes.Add(node);
