...
{
    public class KeenStunShot : CollisionObject, ITrajectory, IMoveable, IUpdatable, ISprite
    {
        ...
        protected override void HandleCollision(CollisionObject obj)
        {
            ...
            else if (obj is ICancellableTrajectory)
            {
                var t = (ICancellableTrajectory)obj;
                t.Cancel();
                if (--_pierce < 0)
                {
                    StopAtCollisionObject(obj);
        ...
        public virtual void Update()
        {
            if (!_shotComplete)
            {
                ...
                var cancellables = collisionObjects.OfType<ICancellableTrajectory>().OfType<CollisionObject>().ToList();
        ...
        private void HandleCollisionByDirection(IEnumerable<CollisionObject> collisions)
        {
            ...
            {
                bool handle = !_shotComplete && ((!handledDebugTileCollision && collision is DebugTile) || !(collision is DebugTile));
                if (handle)
                {
